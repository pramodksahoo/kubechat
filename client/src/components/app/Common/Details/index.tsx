...
const KcDetails = () => {
  ...
  const { config } = kcDetails.useParams();
  const { cluster, resourcekind, resourcename, group = '', kind = '', resource = '', version = '', namespace } = kcDetails.useSearch();
  ...
  document.title = `kubechat - ${resourceInitialData.label.toLowerCase()} - ${resourceData?.subHeading}`;
