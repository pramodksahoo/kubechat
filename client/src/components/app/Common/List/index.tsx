...
export function KcList() {
  ...
  const { config } = kcList.useParams();
  const { cluster, resourcekind, group, kind, resource, version } = kcList.useSearch();
  const commonSearchParams: CommonSearchParams = kcList.useSearch();
  ...
  document.title = `kubechat - ${tableData.instaceType}`;
  return (
    <CreateTable
      clusterName={cluster}
      configName={config}
