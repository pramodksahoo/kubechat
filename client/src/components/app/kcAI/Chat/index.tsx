...
import { ChatMessage, kcAIStoredChatHistory, kcAIStoredModel, kcAIStoredModels } from "@/types/kcAI/addConfiguration";
...
import { getFullTools } from '@/data/KcAi/KcAiToolsSlice';
// import { getFullTools } from '@/data/KcAi/KcAiToolsSlice';
...
import { createPlanFromPrompt } from "@/data/Plans/PlanPreviewSlice";
...
type ChatWindowProps = {
  ...
  kcAIStoredModels: kcAIStoredModels;
  resetChat: () => void
  namespace?: string
}

const ChatWindow = ({ currentChatKey, cluster, config, isDetailsPage, kcAIStoredModels, resetChat, namespace }: ChatWindowProps) => {
  ...
  const [providerList, setProviderList] = useState<{ [uuid: string]: kcAIStoredModel }>({});
      ...
      case "xai":
        return createXai({
          apiKey: providerData.apiKey, baseURL: providerData.url, headers: {
            'HTTP-Referer': 'https://kubechat.com',
            'X-Title': 'Kubechat'
          }
        });
      case "openai":
  ...
  const generateStreamText = async () => {
    ...
    setInput('');
    if (submittedPrompt) {
      dispatch(createPlanFromPrompt({
        prompt: submittedPrompt,
        clusterHint: cluster,
        namespaceHint: namespace,
        metadata: {
          source: isDetailsPage ? "details-chat" : "list-chat",
          config,
  ...
  const storeToChatHistory = useCallback((key: string) => {
    try {
      ...
      if (!kcAIChatHistory[clusterConfigKey]) {
        kcAIChatHistory[clusterConfigKey] = {
          [key]: {
            messages: [],
      ...
      localStorage.setItem('kcAIStoredChatHistory', JSON.stringify(kcAIChatHistory));
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (currentContext?.messages) {
      ...
      if (kcAIStoredModels) {
        setSelectedProvider(currentContext?.provider || kcAIStoredModels.defaultProvider);
      }
    } else {
      setMessages([]);
      if (kcAIStoredModels) {
        setSelectedProvider(kcAIStoredModels.defaultProvider);
      }
    }
          ...
          <div className="flex items-center justify-between px-4 pb-3">
            <div className="flex gap-2">
              <Popover open={open} onOpenChange={setOpen}>
                ...
                <PopoverContent className={`p-0 min-w-[--radix-popover-trigger-width] w-auto`} style={{ 'maxWidth': kcAiChatWindow.width - 50 }} align="start">
                  <Command>
