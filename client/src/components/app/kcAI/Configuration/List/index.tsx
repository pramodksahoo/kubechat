...
const ListConfigurations = ({ setSelectedUUId, setKcAIStoredModelsCollection, isDetailsPage, setShowAddConfiguration }: ListConfigurationsProps) => {
  const kcAIStoredModels = JSON.parse(localStorage.getItem('kcAIStoredModels') || '{}') as kcAIStoredModels;
  const [data, setData] = useState(kcAIStoredModels);
  ...
  const deleteKcAIStoredModels = (uuid: string) => {
    const storedKcAIModel = localStorage.getItem('kcAIStoredModels');
    if (storedKcAIModel) {
      const kcAIModelCollection = JSON.parse(storedKcAIModel) as kcAIStoredModels;
      delete kcAIModelCollection.providerCollection[uuid];
      setKcAIStoredModelsCollection(kcAIModelCollection);
      localStorage.setItem('kcAIStoredModels', JSON.stringify(kcAIModelCollection));
    }
  };
  ...
  const updateDefaultProvider = (uuid: string) => {
    ...
    if (storedKcAIModel) {
      const kcAIModelCollection = JSON.parse(storedKcAIModel) as kcAIStoredModels;
      kcAIModelCollection.defaultProvider = uuid;
      setKcAIStoredModelsCollection(kcAIModelCollection);
      localStorage.setItem('kcAIStoredModels', JSON.stringify(kcAIModelCollection));
      setData((prev) => {
        const cluster = prev;
