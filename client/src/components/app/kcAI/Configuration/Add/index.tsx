...
import { kcAIStoredModels } from "@/types/kcAI/addConfiguration";
import { kcAiModels, resetKcAiModels } from "@/data/KcClusters/kcAiModelsSlice";
import { useAppDispatch, useAppSelector } from "@/redux/hooks";

import { Button } from "@/components/ui/button";
import { ComboboxDemo } from "@/components/app/Common/Combobox";
import { Input } from "@/components/ui/input";
import { KC_AI_PROVIDERS } from "@/constants/kcAi";
...
type AddConfigurationProps = {
  ...
  cluster: string;
  config: string;
  setKcAIStoredModelsCollection: Dispatch<SetStateAction<kcAIStoredModels>>;
}

const AddConfiguration = ({ uuid, setShowAddConfiguration, config, cluster, setKcAIStoredModelsCollection }: AddConfigurationProps) => {
  const dispatch = useAppDispatch();
  const {
    error,
    kcAiModel,
    loading
  } = useAppSelector((state) => state.kcAiModels);
  ...
  const getDefaultState = (uuid?: string) => {
    ...
    if (uuid) {
      const storedKcAIModel = localStorage.getItem('kcAIStoredModels');
      if (storedKcAIModel) {
        defaultState = (JSON.parse(storedKcAIModel) as kcAIStoredModels).providerCollection[uuid] || defaultState;
      }
    }
    return defaultState;
  };
  ...
  const getDefaultCurrentProvider = (uuid?: string) => {
    ...
    const storedKcAIModel = localStorage.getItem('kcAIStoredModels');
    if (storedKcAIModel) {
      return (JSON.parse(storedKcAIModel) as kcAIStoredModels).defaultProvider === uuid;
    }
  };
  ...
  const handleProviderChange = (name: string, providerValue: string | boolean) => {
    const providerDefaultUrl = KC_AI_PROVIDERS.find(({ value }) => value === providerValue)?.providerDefaultUrl || '';
    setFormData(prev => {
  ...
  const storeConfigToLocalStorage = () => {
    const currentUUID = uuid || `kc_provider_${Date.now()}_${Math.random().toString(36).slice(2, 5)}`;

    try {
      const raw = localStorage.getItem('kcAIStoredModels');
      let kcAiModels: kcAIStoredModels = raw ? JSON.parse(raw) : {};

      if (!kcAiModels || typeof kcAiModels.providerCollection === 'undefined') {
        kcAiModels = {
          defaultProvider: '',
          providerCollection: {}
        };
      ...
      kcAiModels = {
        ...
        providerCollection: {
          ...kcAiModels.providerCollection,
          [currentUUID]: formData,
        }
      };

      localStorage.setItem('kcAIStoredModels', JSON.stringify(kcAiModels));
      setKcAIStoredModelsCollection(() => kcAiModels);
      ...
      console.error('kcAIStoredModelsFailure', error);
    }
  };
    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      <div className="p-4 pt-0 flex-1 flex flex-col space-y-4 overflow-auto pr-2">
        <div>
          ...
          <ComboboxDemo data={KC_AI_PROVIDERS} setValue={(value) => handleProviderChange('provider', value)} value={formData.provider} placeholder="Select Provider..." />
        </div>
        <div>
          <Label>URL</Label>
          <Input placeholder="URL to model provider" id="addKcAiConfigUrl" className={cn('shadow-none', error && 'border-destructive focus-visible:ring-destructive')} value={formData.url} onChange={(e) => handleChange('url', e.target.value)} />
        </div>

        {
          ...
          <div>
            <Label>API Key</Label>
            <Input placeholder="Secret Key" id="addKcAiConfigApiKey" className={cn('shadow-none', error && 'border-destructive focus-visible:ring-destructive')} value={formData.apiKey} onChange={(e) => handleChange('apiKey', e.target.value)} />
          </div>
        }
...

        {
          ...
          <div>
            ...
            <Input placeholder="API Version" id="addKcAiConfigApiVersion" className={cn('shadow-none', error && 'border-destructive focus-visible:ring-destructive')} value={formData.apiVersion} onChange={(e) => handleChange('apiVersion', e.target.value)} />
          </div>
        }
        ...
        <div>
          <Label>Model</Label>
          {
            loading ? <Skeleton className="h-9 w-full rounded-md" /> :
              <ComboboxDemo data={kcAiModel} setValue={(value) => handleChange('model', value)} value={formData.model} placeholder="Select Model..." />
          }
          {
        ...
        <div>
          <Label>Alias</Label>
          <Input placeholder="Quick identifier name model" id="addKcAiConfigAlias" className="shadow-none" value={formData.alias} onChange={(e) => handleChange('alias', e.target.value)} />
        </div>
        <div>
