...
import { kcAIStoredChatHistory } from "@/types/kcAI/addConfiguration";
...
const ChatHistory = ({ cluster, config, isDetailsPage, resumeChat }: ChatHistoryProps) => {
  ...
  const [chatHistory, setChatHistory] = useState(kcAIStoredChatHistory);
  const [deletingRowIds, setDeletingRowIds] = useState<Set<string>>(new Set());

  const deleteKcAIStoredChatHistory = (uuid: string) => {
    const kcAIStoredChatHistory = localStorage.getItem('kcAIStoredChatHistory');
    if (kcAIStoredChatHistory) {
      const kcAIStoredChatHistoryCollection = JSON.parse(kcAIStoredChatHistory) as kcAIStoredChatHistory;
      if (kcAIStoredChatHistoryCollection[clusterConfigKey][uuid]) {
        delete kcAIStoredChatHistoryCollection[clusterConfigKey][uuid];
      }
      localStorage.setItem('kcAIStoredChatHistory', JSON.stringify(kcAIStoredChatHistoryCollection));
    }
  };
  ...
  const confirmDelete = (uuid: string) => {
    ...
    deleteKcAIStoredChatHistory(uuid);
  };
    ...
    <div className="flex flex-col h-full">
      {
        (!chatHistory[clusterConfigKey] || Object.keys(chatHistory[clusterConfigKey]).length === 0) ?
          ...
          <>
            <div className="p-4">
              <h3 className="text-lg font-medium pb-1">kcAI Chat History</h3>
